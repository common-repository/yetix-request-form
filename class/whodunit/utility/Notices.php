...
class Notices{
	...
	protected $transient = 'yetix_request__notices';
	protected $notices   = false;
	protected $user_id;

	static function get_instance(){
		if( !self::$instance ){ self::$instance = new Notices; }
	...
	public function echo_notices( $trash = true ){
		if( $this->notices ){
			...
			if( isset( $notices[ $this->user_id ] ) ){
				foreach( $notices[ $this->user_id ] as $notice ){
					echo '<div class="notice notice-'.esc_attr( $notice[ 'class' ] ).' is-dismissible"><p>'
						.esc_html( $notice[ 'notice' ] )
						.'</p></div>';
				}
				if( $trash ){ $this->trash( $notices, $this->user_id ); }
			}
		}
	}
