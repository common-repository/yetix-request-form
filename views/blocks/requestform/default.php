<?php
	/*
	 * Request Form block default template
	 * this can be overwritten if you put a default.php file into a ./template_yetix/blocks/requestform/ directory of your theme
	...
	$class_name     = ( isset( $className ) ) ? esc_attr( $className ) : false ;
	//attributes
...
?>
	<div id="<?php echo esc_attr( $uniq_id ); ?>" class="<?php echo ( $class_name ) ? esc_attr( $class_name ).' ' : ''; ?>yetix wp-block-yetix-requestform yetix-requestform<?php echo ( $align ) ? ' '.esc_attr( $align ): ''; ?><?php echo ( $is_admin ) ? ' '.'wp-block-yetix-waiting-setup': ''; ?>">
...
<?php
			\whodunit\utility\Helpers::zendesk_sorted_fields( $is_admin, true );
			echo '<p>'.esc_html__( 'Required fields are marked *', 'yetix-request-form' ).'</p>';
?>
			<button class="yetix-requestform submit ui button" type="submit" <?php echo ( $is_admin ) ? ' disabled' : ''; ?>><?php esc_html_e( 'Send', 'yetix-request-form' ); ?></button>
		</form>
	</div>
<?php
	elseif( $is_admin || current_user_can( 'yetix_request__manage_plugin_options' ) ):
?>
